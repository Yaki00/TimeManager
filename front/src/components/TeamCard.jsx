...
export const TeamCard = ({ team, handleDelete }) => {
	return (
		<CardContainer>
			<CardStyle>
				<Link to={`/teams/${team.id}`} state={{ id: team.id }} style={{ textDecoration: 'none', color: 'inherit', display: 'flex', flexDirection: 'column', flex: 1 }}>
					<CardHeader>
						<TeamInfo>
							<TeamName>
								<TeamOutlined />
				...
				<CardFooter>
					<Link to={`/teams/${team.id}`} state={{ id: team.id }} style={{ textDecoration: 'none' }}>
