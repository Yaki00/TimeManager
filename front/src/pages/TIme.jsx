...
export const Time = () => {
  ...
  const {
    ...
    error: leavesError,
  } = useGetLeavesByUserId(userId);
  const { deleteLeaveAsync, isLoading: isDeleting } = useDeleteLeave();
  ...
  const leaveStats = React.useMemo(() => {
    ...
    const approvedLeaves = safeUserLeaves.filter(
  ...
  const onFinish = async (value) => {
    ...
    const response = await createLeaveAsync({
      ...
      dayLeave:
        dayjs(value.dateRange[1]).diff(dayjs(value.dateRange[0]), "day") + 1,
    });
    if (response.status !== "error") {
      const typeLabel = leaveTypeLabels[value.leaveType] || "Demande";
      message.success(
        `${typeLabel} demandé${
          value.leaveType === "remote" ? "" : "e"
        } du ${dayjs(value.dateRange[0]).format("DD/MM/YYYY")} au ${dayjs(
          value.dateRange[1]
        ).format("DD/MM/YYYY")}`
      );
  ...
  const handleDeleteLeave = useCallback(
    async (leaveId) => {
      try {
        await deleteLeaveAsync(leaveId);
        message.success("Demande de congé supprimée avec succès");
      } catch (error) {
        message.error(
  ...
  const columns = useMemo(
    () => getColumns(handleDeleteLeave),
    [handleDeleteLeave]
  );

  if (isLoadingLeaves || isLoading || isDeleting)
        ...
        <ScrollableContent>
          <div style={{ padding: "40px", textAlign: "center" }}>
            <Alert
              message="Erreur"
              description={
                leavesError.message || "Erreur lors du chargement des données"
              }
              type="error"
              showIcon
            />
                ...
                <LeaveSummary>
                  ...
                  <div className="leave-row">
                    ...
                    <span className="value">
                      {leaveStats.remainingDays} jours
                    </span>
                  </div>
                  <StyledProgress
              ...
              <Section>
                ...
                <LeaveSummary>
                  ...
                  <div className="leave-row">
                    ...
                    <span className="value">
                      {leaveStats.remainingRemoteDays} jours
                    </span>
                  </div>
                  <StyledProgress
                    percent={
                      (leaveStats.remoteDays / leaveStats.totalRemoteDays) * 100
                    }
                    showInfo={false}
                  />
                </LeaveSummary>
