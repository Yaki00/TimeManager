...
export const Employer = ({ dateRange }) => {
  ...
  const avgPresence =
    kpiData.personalAttendance?.length > 0
      ? (
        kpiData.personalAttendance.reduce((acc, curr) => acc + curr.rate, 0) /
          kpiData.personalAttendance.length
      ).toFixed(1)
      : 0;
          ...
          <ChartContainer>
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={kpiData.arrivalDeparture || []}>
                ...
                <Tooltip
                  formatter={(value) =>
                    `${Math.floor(value)}h${Math.round((value % 1) * 60)
                      .toString()
                      .padStart(2, "0")}`
                  }
                  contentStyle={{
